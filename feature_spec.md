...
| 当前状态 | `v0.1.5 Phase 1, Task 1` 至 `v0.1.5 Phase 1, Task 8` 已完成；针对性测试 98 项、全量测试 269 项通过，`flutter analyze` 零问题。 |
...
| v0.1.5 Phase 1, Task 7 | 实时协议与重置边界接入 | 接入 Buff Topic、模块字段和共享状态；通知运行时独立重放 MQTT 缓存；比赛边界清比赛临时状态，MQTT 会话结束时额外清模块状态 | `lib/features/dashboard/logic/stream_providers.dart`, `lib/features/dashboard/logic/notification_providers.dart`, `test/notification_runtime_test.dart`, `test/notification_runtime_widget_test.dart` | `[x]` |
...
**v0.1.5 验收标准：** 官方通知规则档案与 17mm/42mm 伤害基线准确；斩杀线结合当前操作手武器、目标血量和攻防 Buff；敌方复活可区分普通免费、加速免费、付费和方式不确定；任一明确离线模块在侧栏持续可见；新小局、离开比赛阶段和身份变化清理比赛临时状态但保留 MQTT 连接级模块状态，MQTT 离开 connected 时额外清理模块状态；自定义图传与操作面板保持不变。

**v0.1.5 验收结果（2026-07-22）：** 官方规则版本更新为 `2.0.0`，敌方普通复活为 INFO、付费复活为 CRITICAL，17mm/42mm 默认伤害为 20/200；斩杀线按操作手身份选择 17mm、42mm 或工程撞击并计算攻防 Buff；敌方复活按普通免费、基地低血量、补给区或原因不确定的加速免费、付费和方式不确定分类，并遵循不确定事件的抑制设置；Buff、比赛倒计时和敌方基地血量仅在 Protobuf 必要字段明确出现时参与判定；复活设置页使用与运行时一致的四类术语，并明确基地阈值读取敌方基地血量；模块状态面板在任一明确离线模块存在时持续显示，全部恢复后切回事件时间轴，首次明确离线与已观测到的在线转离线使用不同说明；恢复事件会先关闭持续告警，比赛重置会清理可见通知、冷却和 UDP 采样基线但保留历史。新小局、离开比赛阶段（包括异常结束兜底）和身份变化只清理比赛临时状态，已明确离线模块及其去重 baseline 跨比赛保留；仅 MQTT 离开 connected 时额外清理连接级模块状态。MQTT 接收边界为每条消息保存原始接收时刻和连接 generation，缓存、Protobuf 信封与通知运行时沿用同一 generation；共享 `mqttMessageProvider` 与通知运行时分别创建解析订阅，因此即使自动导出等消费者先激活并消费共享缓存，通知运行时仍会独立完整重放当前 generation 缓存；旧 generation 继续被拒绝。敌方详情模式已移除最低血量选敌算法和指挥性集火建议，仅保留中性状态展示；本版本未修改自定义图传行为或操作面板。针对性测试 98 项通过，`flutter analyze` 零问题，全量测试 269 项通过。
...
| 版本 | 日期 | 变更摘要 |
|------|------|----------|
| 0.1.5 | 2026-07-22 | 新增 Dashboard 模块状态持续显示面板，并接入 Buff 与模块实时协议状态；修复 Protobuf 必要字段 presence、复活基地证据三态、不确定事件抑制、恢复告警关闭顺序、运行时重置，以及 MQTT 原始接收时刻与连接 generation 贯穿缓存、解析和通知运行时的会话隔离；修复 通知运行时后启动时无法重放已被共享 Provider 消费的当前 generation 缓存，并保存、释放独立解析订阅；修复 模块状态被比赛 reset 误清，改为跨新小局、离场和身份变化保留且仅在 MQTT 会话结束时清理；修复 首次明确离线与在线转离线使用相同说明的问题，并同步普通免费、加速免费、付费、方式不确定及敌方基地阈值等设置说明；优化 官方规则版本、复活严重级别和 17mm/42mm 默认伤害；移除 敌方最低血量选敌算法和指挥性集火建议，仅保留中性状态展示；文档 完成 98 项针对性测试、269 项全量测试和零问题静态分析的发布收口 |
...
*文档版本：v2.23（完成 v0.1.5 启动顺序与模块生命周期最终门禁修复）*
