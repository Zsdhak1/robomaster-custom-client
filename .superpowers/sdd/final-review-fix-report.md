...
最终审查及合并门禁复审发现的问题均已闭环。独立最终验证为聚焦回归 88/88、`flutter analyze` 无问题、全量回归 268/268。保留的 concern 仅为 `notification_providers.dart` 的 Minor 可维护性问题；不在发布前为文件长度做高风险重构。
...
- `ProtobufEnvelope` 沿用接收边界提供的时间和 generation；通知运行时仅接受当前已连接 generation，时间基线作为附加检查。
...
GREEN：`test/notification_runtime_widget_test.dart` 5/5 通过，覆盖旧 generation 的缓存比赛状态无法恢复、当前 generation 协议事件仍可处理；纯 fence 测试覆盖 disconnected、缺基线、旧时间戳、generation 不匹配和当前会话接受。

### 8. 首次离线与设置说明语义（Important）

- 首次明确离线写为“模块明确上报离线状态”，只有已经观测到在线后再离线才写“由在线变为离线”。
- 设置页准确说明普通免费、加速免费、方式不确定、付费复活，以及首次明确离线的事件覆盖范围。
...
- 聚焦回归：exit 0，88/88，`All tests passed!`。
- 静态分析：exit 0，`No issues found!`。
- 全量回归：exit 0，268/268，`All tests passed!`。
