...
最终审查的 1 项 Critical、5 项 Important 和 UDP sampler Minor 均已闭环。独立最终验证为聚焦回归 71/71、`flutter analyze` 无问题、全量回归 266/266。保留的 concern 仅为 `notification_providers.dart` 568 行的 Minor 可维护性问题；按 brief 不在发布前为文件长度做高风险重构。
...
- 统一 `_resetMatchState()` 同时清规则引擎、共享模块 baseline、通知运行时、连接质量、UDP 窗口与部署状态。
...
- `start()` 从 `mqttConnectionStateProvider` 当前值初始化 runtime 和 tracker；启动时已连接会建立同一 `DateTime.now()` 时钟口径的接收基线。
- 非 connected、缺连接基线或 `envelope.timestamp` 早于当前连接时间的信封全部拒绝；等于连接时间可接受。
- 从 connected 离开时清 `_gameStatus`、连接/消息时间基线并统一 reset；重连前后的血量不会按旧 in-match 状态处理。

RED：移除断线 `_gameStatus` 清理后，重连第一份血量产生 4 个旧比赛 kill-line 通知；启动时已 connected 但 tracker 未初始化时，断线通知数量为 0 而非 1。

GREEN：`test/notification_runtime_widget_test.dart` 4/4 通过，覆盖启动时已连接、断线通知、断线清旧比赛状态和重连隔离；纯 fence 测试覆盖 disconnected、缺基线、旧时间戳、等时刻接受。
...
主代理在全部实现、机械 lint 修复和格式化后独立运行：

```text
flutter test --reporter expanded test/dashboard_notification_controller_test.dart test/combat_buff_tracker_test.dart test/notification_runtime_test.dart test/notification_runtime_widget_test.dart test/notification_rule_engine_test.dart test/robot_status_list_test.dart
flutter analyze
flutter test --reporter expanded
```

- 聚焦回归：exit 0，71/71，`All tests passed!`。
- 静态分析：exit 0，`No issues found!`。
- 全量回归：exit 0，266/266，`All tests passed!`。
...
1. `lib/features/dashboard/logic/notification_providers.dart` 当前 568 行，仍有 Minor 级可维护性 concern。审查与 266 项全量回归未发现功能阻塞；按 brief 留待后续独立重构任务。
