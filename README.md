...
一个面向 **RoboMaster 2026 机甲大师超级对抗赛** 的自定义客户端监控应用。基于 Flutter 构建，对接官方自定义客户端链路与 0x0310 自定义图传：

- **MQTT 3333** — 控制指令、比赛状态与事件（Protobuf 序列化）
- **UDP 3334** — HEVC(H.265) AnnexB 视频流（自定义字节偏移分片）
- **CustomByteBlock / 0x0310** — MQTT 承载 H.264 字节块，经独立 TCP 桥进入视频解码后端

支持实时比赛监控、双图传低延迟渲染、JSON 数据导出/回放、多客户端数据合并，以及基于 GitHub 的远程记录同步。
...
| **自定义图传** | 0x0310 `CustomByteBlock` H.264 接收、MPEG-TS 封装、准星叠加、解码调试面板 |
